...
## all.set for subset A, B, C
## Allspecies=w/ troco || species=w/o troco for lipids =====IMPORTANT=====
{
setwd("C:/Dropbox/Workshop2013/Work/R/sam/")
all.set <- read.table("all.set.txt", sep="\t", header=F)
setwd("C:/Dropbox/Workshop2013/Work/R/datasets/weights/")
...
dim(logs)	#15x1444
Allspecies <- logs
species <- logs[-c(4:6),]	## remove trocophores
}
## LOAD neutral and polar lipids
{
setwd("C:/Dropbox/Workshop2013/Work/R/datasets/")
...
## Load all GO sets (From Fishers enrichment analysis)
{
...
## PLOTTING
a=c(1:35)			## Choose which Transription factors to DRAW
{				## Empty plot
...
result <- rda(Allspec~Egg+Juvenile+Veliger,stage)
{				## diagnostics
vif.cca(result)
result$CCA$tot.chi/result$tot.chi			## Total variance
RsquareAdj(result)
...
cat("\n\nIf constrained inertia in permutation test is lower than the observed inertia: Constraints are then significant")
## Start with unconstrained (option 1)
extended <- rda(Allspec ~. , Allstage)			# Dont Manipulate
reduce0 <- rda(Allspec ~1, Allstage)				# ~1 for unconstrained
reduce <- step(reduce0, scope=formula(extended), test="perm")
reduce
reduce$anova
vif.cca(reduce)
...
## Extract to blast2GO
setwd("C:\\Blast2GO\\")
write.table(cluster11,"cluster11.txt",sep="\t",quote=F)
